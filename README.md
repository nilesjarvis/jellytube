...
The Watch page prepares a direct stream when the browser can play it and falls back to Jellyfin HLS when needed. For HEVC sources, JellyTube probes the exact profile/level, HDR output range, Dolby Vision profile, fMP4 HLS support, and audio codecs before advertising a source-quality remux profile to Jellyfin. Compatible playback copies the original HEVC frames into fMP4 HLS, preserving resolution, bit depth, BT.2020/PQ signaling, and supported dynamic HDR metadata; AC-3/E-AC-3 audio is copied when supported, otherwise only audio is converted. MSE browsers such as Firefox can use an explicitly HDR10-compatible Dolby Vision 8.1 base layer without claiming Dolby Vision output. Unsupported browsers and SDR displays retain the existing tone-mapped H.264 fallback. Videos with multiple audio streams expose an audio-track menu beside captions; the semantic language/title choice is saved per Jellyfin server and user, carried into Jellyfin playback negotiation/reporting, and resolved safely when later videos have different stream indexes or available tracks. Alternate embedded tracks use HLS because a static direct-play URL cannot select a non-container-default track. The aspect-ratio menu preserves the source by default, can stretch incorrectly formatted media to 4:3, 16:9, or 21:9 without cropping, and retains the existing 21:9 crop mode. The player reports progress to Jellyfin, resumes from saved playback position, persists volume/mute state, supports seeking and fullscreen, and can continue through episode shelves or mix queues. Its recommendation rail projects ranked items only after excluding the current series, so an ordered show appears once with the user's progress action and cannot reappear beside one of its own episodes.
