...
- Node.js 20+ for source and manual installations; the one-command Linux installer supplies its own verified Node.js runtime
...
#### HEVC And HDR Playback

JellyTube does not add a video decoder or turn SDR media into HDR. Before offering source-quality HEVC playback, it checks the current source and client together: the exact HEVC profile and level, resolution, frame rate, bitrate, HDR transfer function, Dolby Vision profile, fMP4 HLS support, audio codecs, and whether the browser reports a high-dynamic-range output. This is intentionally runtime and source specific instead of relying on a browser-name compatibility list.

| HEVC source | Compatible client result | Player badge |
| --- | --- | --- |
| SDR | Direct play or source-quality fMP4 HLS remux. | None |
| HDR10 | Original 10-bit video with BT.2020/PQ signaling is passed through. Generic PQ metadata is recognized as HDR10 even when Jellyfin reports only `HDR`. | HDR10 |
| HDR10+ | The original HEVC frames and HDR10+ metadata are copied. Whether the browser, OS, and display apply the dynamic metadata remains client dependent. | HDR10+ |
| HLG | The original HLG range is passed through when the client reports compatible HEVC, HLG, and HDR output support. | HLG |
| Dolby Vision Profile 8 with compatibility ID 1 | Native Dolby Vision is used only after an exact Dolby Vision probe succeeds. Otherwise, a compatible MSE client may use the stream's independently decodable HDR10 base layer. | Dolby Vision or HDR10, matching the active path |
| Other Dolby Vision profiles | Used only when the native playback path explicitly accepts the exact Dolby Vision codec. They are not treated as ordinary HEVC or assumed to contain a safe HDR10 base layer. | Dolby Vision, or no badge after SDR fallback |

Typical native-HLS clients such as Safari can expose Dolby Vision when the codec and container signaling are accepted. MSE clients such as Firefox can use HEVC fMP4 HLS and, for Dolby Vision Profile 8.1, the explicitly HDR10-compatible base layer; JellyTube does not claim native Dolby Vision on that route. On Linux, the browser, GPU decoder, driver, compositor, monitor connection, and display must collectively expose HEVC and HDR support. The headless machine serving JellyTube does not determine the display mode.

There are two lossless video delivery paths:

- **Direct Play** sends the existing MP4 when its container, default audio track, and exact video format are accepted. A progressive MP4 is labeled Dolby Vision only when it has a Dolby Vision sample entry such as `dvh1` or `dvhe`; an `hev1`-tagged Dolby Vision Profile 8.1 file is conservatively identified as its HDR10-compatible base unless a valid native Dolby Vision HLS path is negotiated.
- **Direct Stream** asks Jellyfin to repackage the source as fragmented MP4 HLS. Jellyfin copies the HEVC video bitstream rather than decoding and re-encoding it, so resolution, source compression, bit depth, color signaling, and in-band HDR metadata are retained. This uses Jellyfin's normal playback endpoint and ffmpeg process; it requires no Jellyfin code or media-file changes, though the account's existing direct-stream and transcoding permissions still apply.

AAC audio can accompany the remux. AC-3 and E-AC-3, including an E-AC-3 Atmos track, are copied only when the client reports fMP4 support for them. Otherwise Jellyfin may convert audio to AAC and may reduce it to stereo while leaving the video untouched. Selecting a track or subtitle mode that must be burned into the picture requires a full video transcode.

Large HDR MP4 files are preferentially sent through the Direct Stream path when Jellyfin reports a file size of at least 4 GiB or 12 or more streams. Such files often place a large MP4 index at the end, which can make progressive browser startup appear frozen. Jellyfin can read that index locally and produce a small HLS initialization segment without re-encoding the video.

Media cards display a source badge for Dolby Vision, HDR10+, HDR10, HLG, or generic HDR, making the file's available format visible before playback. This card badge describes the source, not a promise that the current client can render it. JellyTube loads the stream metadata for cards near the viewport in small batched requests instead of adding full media details to the catalog feed.

If any required video, HLS/MSE, Dolby Vision, or HDR-output check fails, JellyTube keeps Jellyfin's H.264 HLS fallback. Jellyfin then tone-maps HDR to SDR, so the original HDR output and dynamic metadata are not delivered. The active HDR/Dolby badge is hidden for that transcoded path. During source-quality playback it appears at the top right with the rest of the player controls, and therefore only while the player is hovered, focused, or otherwise showing its controls. The player badge confirms the format JellyTube is delivering without video transcoding; browser APIs cannot independently prove which dynamic metadata the physical display ultimately applies.
